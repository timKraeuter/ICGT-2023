...
The BPMN execution semantics are described using the concept of \textit{tokens} \cite{objectmanagementgroupBusinessProcessModel2013}, which can be located at sequence flows and specific flow nodes.
...
Since our approach is based on a \gls*{hot} from BPMN to \gls*{gt} systems, we generate a \textit{start graph} and \textit{\gls*{gt} rules} for a given BPMN model.
...
The upper rule describes how MITEs interact with MICEs.
...
The middle rule shows how MITEs trigger new process instances when interacting with \textit{message start events} (MSEs).
...
The bottom part in \autoref{fig:messageEventTemplates} depicts the rule generation template for MICEs.
...
The models include realistic business process models (001, 002, and 020), see \cite{houhouFirstOrderLogicVerification2022}.
...
The benchmarks were run on Windows 11 (AMD Ryzen 7700X processor, 32 GB RAM) using Groove version 5.8.1, see \cite{krauterArtifactsICGT2023}. 
