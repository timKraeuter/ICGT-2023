...
All the rule generation templates can be found in \cite{krauterArtifactsTERMGRAPH2022}.
...
The termination rule in Groove is given as an artifact \cite{krauterArtifactsTERMGRAPH2022}.
...
The atomic property \textsf{Unsafe} is true if two tokens of one process snapshot point to the same sequence flow. This atomic proposition \textit{Unsafe} is depicted in \cref{fig:unsafe}; Groove rules for all the atomic propositions are included in \cite{krauterArtifactsTERMGRAPH2022}.
...
Both properties can be checked using our implementation \cite{krauterArtifactsTERMGRAPH2022}.
...
The tool is open-source, publicly available, and does not require any installation \cite{krauterArtifactsTERMGRAPH2022}.
...
% Evaluation
To evaluate the correctness of our implementation, we created a comprehensive test suite, which demonstrates correct rule generation for the implemented BPMN constructs \cite{krauterArtifactsTERMGRAPH2022}.
