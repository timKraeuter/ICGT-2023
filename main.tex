...
% We keep the execution metamodel and the execution typegraph separate (see \autoref{fig:approach}) even tho they contain the same information because of two reasions.
% First, transforming a class diagram to an attributed type graph is not always straight forward???
% Second, the typegraph must be understood by the graph transformation tool to be used
...
We cannot use the concrete syntax to define the \textsf{Unsafe} proposition because it must apply to any BPMN model.
Our concrete syntax is always used alongside a given BPMN model.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=.9\textwidth]{images/impl_very_short.png}
...
For example, \cite{dijkmanSemanticsAnalysisBusiness2008} formalize a subset of \gls*{bpmn} elements by defining a mapping to Petri Nets, which is conceptually close to our \gls*{hot}-based formalization.
% Difficulties with Petri net encodings
Encoding basic \gls*{bpmn} modeling elements into Petri Nets is generally simple, but for some advanced elements, it can be complicated to define \cite{hofstedeWorkflowPatternsExpressive2002}.
For example, representing \textit{termination end events} and \textit{interrupting boundary events}, which interrupt a running process, is usually not supported because of the complexity of managing non-local propagation of tokens in Petri Nets \cite{corradiniFormalApproachAnalysis2021}.
We solve these situations by using nested graph conditions, for example, to remove all tokens when reaching a termination end event.
