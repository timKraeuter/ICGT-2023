...
Our approach is based on a \gls*{hot} from BPMN models to graph transformation systems.
As proof of concept, we have implemented our approach in an open-source web-based tool.

\keywords{BPMN \and Graph Transformation \and Model transformation \and Model Checking \and Formalization}
\end{abstract}
%
%
%
...
To this end, we propose a formalization that, compared to other approaches, covers most of the BPMN features.

% Describe the approach and highlight that it can be generalized to other languages which have state and state changing-elements.
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.7\textwidth]{images/bpmn_semantics-overview.pdf}
    \caption{Overview of the approach}
    \label{fig:approach}
\end{figure}

Our approach is based on a \gls*{hot} from BPMN process models to graph transformation systems using rule generation templates.
...
Thus, our approach features a new graph transformation system, i.e., graph transformation rules and a start graph for each BPMN process model.
...
Thus, they are simpler than the general rules in \cite{vangorpVisualTokenbasedFormalization2013} since they only implement one BPMN feature in a certain process model, not in all possible scenarios.
...
Furthermore, we outline the theoretical background behind our application of graph transformations.
\subsection{BPMN}
% Introduce BPMN
\autoref{fig:bpmnMetamodel} depicts the structure of BPMN process models with the corresponding concrete syntax BPMN symbols contained in clouds.
...
Since our approach is based on a \gls*{hot} from BPMN to graph transformation systems, we generate a \emph{start graph} and \emph{graph transformation rules} for a given BPMN process model.
...
The \gls*{hot} generates one or more graph transformation rules for each \textsf{FlowNode} in a BPMN model.
...
The atomic property \textsf{Unsafe} is true if two tokens of one process snapshot point to the same sequence flow. This atomic proposition \textit{Unsafe} is depicted in \autoref{fig:unsafe}; Groove rules for all the atomic propositions are included in \cite{krauterArtifactsICGT2023}.
...
Finally, our tool can generate a graph transformation system for the supplied \gls*{bpmn} model and run model checking in Groove.
% Evaluation
...
However, each approach supports a different subset of the BPMN features.
...
Van Gorp et al. \cite{vangorpVisualTokenbasedFormalization2013} cover a large part of the BPMN semantics including error and cancellation events which we did not realize yet.
...
Referring to \autoref{tab:supportedfeatures}, we conclude that our formalization is comprehensive but still lacks support for some of the more advanced event types.
Implementing the missing event types is feasible, as shown in \cite{vangorpVisualTokenbasedFormalization2013}.

\section{Conclusion \& future work} \label{sec:conclusion}
% TODO: Summarize the contributions of the paper.
% Summary
The approach presented in this paper utilizes a \gls*{hot} from BPMN models to graph transformation systems. 
For each BPMN model, we automatically generate a start graph and a set of graph transformation rules.
In this way, the graph transformation rules get simpler while the complexity is shifted to the \gls*{hot}.
...
% More semantics
First, we intend to extend our formalization to support even more BPMN features, for example, error, cancel, and compensation events.
% Proper evaluation
