...
Our approach has been incorporated into a user-friendly, open-source web-based tool, the \textit{\gls*{bpmn} Analyzer}, which can be used online without needing installation.
...
% To summarize, our paper makes two main contributions of practical nature.
% First, we conceptualize a new approach utilizing a \gls*{hot} to generate \gls*{gt} rules instead of providing a fixed set of \gls*{gt} rules to formalize the semantics of a behavioral language.
% Second, we apply our approach to \gls*{bpmn}, resulting in a formalization covering most \gls*{bpmn} elements, which facilitates 
