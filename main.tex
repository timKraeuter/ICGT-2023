...
First, we describe the \gls*{bpmn} semantics formalization using graph transformation systems (\autoref{sec:formalization}) before explaining how this can be utilized for model checking BPMN-specific and custom properties (\autoref{sec:modelChecking}).
...
\subsection{BPMN}
% Introduce BPMN
BPMN is a popular standard notation to define intra- and inter-organizational workflows.
\autoref{fig:bpmnMetamodel} depicts the structure of BPMN process models and the corresponding BPMN symbols contained in clouds.
...
To give an intuition about the transformation, we will first describe example results, i.e., generated rules for a NSE, a task, a \textit{message intermediate catch event} (MICE), and a \textit{message intermediate throw event} (MITE) (see \autoref{fig:bpmnConstructsOverview}).
Afterward, we will explain how these---and other---rules are created by our higher-order transformation.

\autoref{fig:gtRuleAbstract} depicts an example graph transformation rule ($L \to R$) for a NSE in abstract syntax.
...
Both properties can be checked using our implementation \cite{krauterArtifactsICGT2023}.
To fully check Soundness, we also need to check (ii) \textit{Proper Completion} and (ii) \textit{No Dead Activities}.
The information needed to check these properties can be found in the graph transformation system's state space.
