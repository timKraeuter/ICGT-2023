...
%Frequently used start event triggers are message and signal start events (see MSE and SSE in \autoref{fig:bpmnfeaturesOverview}).
...
%Furthermore, we consider allowing the modeler to define a start graph the same way as she defines atomic propositions for custom properties (see \autoref{subsec:customProperties}).
...
%suggestion
In order to provide a better understanding of the transformation process, we will begin by presenting two example results, namely the generated rules for a NSE and a task (as shown in \autoref{fig:bpmnfeaturesOverview}). Following this, we will delve into an explanation of how our \gls*{hot} creates these rules as well as others.
...
This rule template generates the sample rule or the rigth of in \autoref{fig:gtRuleConcrete}.
